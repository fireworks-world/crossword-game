             ...
             )
{
    ...
    if ( !!globals->langFileName && !globals->locInst ) {
        inst = ceLoadResFile( globals->langFileName );
        if ( !inst ) {          /* load didn't work */
...
LRESULT CALLBACK
WndProc( HWND hWnd, UINT message, const WPARAM wParam, const LPARAM lParam )
{
    LRESULT result = 0;
    int wmId;
    XP_Bool draw = XP_FALSE;
...
# endif
        ...
        case WM_CHAR: {
            ...
            if ( wParam == 0x08 ) {
                tmpWParam = XP_CURSOR_KEY_DEL;
#ifdef KEYBOARD_NAV
            } else if ( wParam == ' ' ) {
                tmpWParam = XP_RAISEFOCUS_KEY;
#endif
            }
            draw = board_handleKey( globals->game.board, tmpWParam, &handled )
                || board_handleKey( globals->game.board, tmpWParam - ('a'-'A'),
                                    &handled );
        }
            break;
